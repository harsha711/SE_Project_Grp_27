  ...
  try {
    ...
    fs.createReadStream(csvPath)
      ...
      .on('data', (row) => {
        ...
        if (!row.Company || row.Company.trim() === '' || !row.Item || row.Item.trim() === '') {
          return;
        }
        ...
        const item = {
          ...
          weightWatchersPoints: parseNumber(row['Weight Watchers Pnts']),
        };
      ...
      .on('end', async () => {
        try {
          ...
          if (items.length > 0) {
            ...
            const sampleItems = await FastFoodItem.find({ calories: { $ne: null } }).limit(5);
            console.log('\nSample items with nutrition data:');
            sampleItems.forEach((item) => {
              console.log(`  - ${item.company}: ${item.item}`);
              console.log(`    Calories: ${item.calories}, Protein: ${item.protein}g, Carbs: ${item.carbs}g`);
            });
          }
