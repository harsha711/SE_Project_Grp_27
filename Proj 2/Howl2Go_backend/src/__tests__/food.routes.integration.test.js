...
test('POST /api/food/parse - returns 400 when query is missing', async () => {
  ...
  assert.equal(response.body.error, 'Query parameter is required and must be a string');
});
...
if (hasGroqApiKey) {
  ...
  test('POST /api/food/parse - parses complex query with multiple criteria', async (t) => {
    ...
    const response = await request(app)
      .post('/api/food/parse')
      .send({ query: 'at least 30g of protein, less than 500 calories, and low carbs' })
      .expect(200);
