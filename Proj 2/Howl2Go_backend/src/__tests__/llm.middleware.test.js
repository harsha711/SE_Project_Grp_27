...
import { parseLLMQuery, buildMongoQuery, validateCriteria } from '../middleware/llm.middleware.js';
...
test('parseLLMQuery - returns 400 when query is missing', async () => {
  ...
  assert.equal(res.jsonData.error, 'Query parameter is required and must be a string');
