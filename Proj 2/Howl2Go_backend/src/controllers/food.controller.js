...
export const getFoodStats = async (req, res) => {
  try {
    ...
    const stats = {
      count: items.length,
      averages: {
        calories: Math.round(items.reduce((sum, item) => sum + (item.calories || 0), 0) / items.length),
        protein: Math.round(items.reduce((sum, item) => sum + (item.protein || 0), 0) / items.length),
        fat: Math.round(items.reduce((sum, item) => sum + (item.total_fat || 0), 0) / items.length),
        carbs: Math.round(items.reduce((sum, item) => sum + (item.total_carb || 0), 0) / items.length)
      },
      ranges: {
        calories: {
