...
const importData = async () => {
    try {
        ...
        const csvPath = path.join(
            __dirname,
            "../../../data/fast-food/FastFoodNutritionMenuV4.csv"
        );
        ...
        fs.createReadStream(csvPath)
            ...
            .on("data", (row) => {
                ...
                const item = {
                    ...
                    iron: parseNumber(row["Iron (mg)"]),
                    potassium: parseNumber(row["Potassium (mg)"]),
                    magnesium: parseNumber(row["Magnesium (mg)"]),
                    calcium: parseNumber(row["Calcium (mg)"]),
                    vitaminA: parseNumber(row["Vitamin A (mg)"]),
                    vitaminC: parseNumber(row["Vitamin C (mg)"]),
                    vitaminD: parseNumber(row["Vitamin D (mg)"]),
                };
            ...
            .on("end", async () => {
                try {
                    ...
                    if (items.length > 0) {
                        ...
                        console.log(
                            `\n✅ Successfully imported ${result.length} fast food items with micronutrient data`
                        );
                        ...
                        // Show sample with nutrition data including micronutrients
                        ...
                        sampleItems.forEach((item) => {
                            ...
                            console.log(
                                `    Iron: ${item.iron}mg, Calcium: ${item.calcium}mg, Vitamin C: ${item.vitaminC}mg`
                            );
                        });

                        // Show micronutrient statistics
                        const itemsWithMicronutrients = await FastFoodItem.countDocuments({
                            $or: [
                                { iron: { $ne: null } },
                                { potassium: { $ne: null } },
                                { magnesium: { $ne: null } },
                        ...
                        console.log(
                            `\nItems with micronutrient data: ${itemsWithMicronutrients}/${result.length}`
                        );
                    }
